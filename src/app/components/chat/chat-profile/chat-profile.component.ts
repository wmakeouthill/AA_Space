...
export class ChatProfileComponent implements OnInit, OnDestroy {
  ...
  error: string | null = null; // Renamed from uploadError to error to match usage
  ...
  ngOnInit(): void {
    ...
    // Adiciona listener para eventos de atualização de imagem
  ...
  private handleProfileImageUpdate = () => {
    console.log('[PROFILE] Evento de atualização de imagem detectado');
    this.loadUserProfile();
  }

  // Método para notificar outros componentes sobre a atualização da imagem
  notifyImageUpdated(): void {
    // Cria um evento personalizado para notificar a atualização da imagem
    const event = new CustomEvent('profile:imageUpdated', {
      detail: { timestamp: Date.now() }
    });
    window.dispatchEvent(event);
    console.log('[PROFILE] Evento de atualização de imagem disparado');
  }
  ...
  formatImageUrl(imagePath: string): string {
    ...
    if (!ChatProfileComponent.imageTimestamps[imagePath]) {
      ChatProfileComponent.imageTimestamps[imagePath] = Date.now();
    }
    
    // Retornar URL completa com parâmetro de cache-busting consistente
    return `${apiOrigin}${imagePath}?t=${ChatProfileComponent.imageTimestamps[imagePath]}`;
  }
  ...
  uploadProfileImageFromLocalStorage(imageBase64: string): void {
    ...
    this.profileService.uploadProfileImageBase64(imageBase64).subscribe({
      next: (response) => {
        console.log('[PROFILE] Imagem enviada com sucesso:', response);
        this.profileImage = response.profileImage;
        localStorage.removeItem('user_profile_image'); // Remove do localStorage após o upload
        this.isUploading = false;
        
        // Notificar outros componentes sobre a atualização da imagem
        this.notifyImageUpdated();
      },
      error: (error) => {
        console.error('Erro ao enviar imagem para o servidor:', error);
        this.error = error.message || 'Erro ao enviar imagem para o servidor';
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files[0]) {
      ...
      if (!file.type.match(/image\/(jpeg|jpg|png|gif)/)) {
        this.error = 'O arquivo deve ser uma imagem (JPEG, PNG ou GIF).';
        return;
      }
      
      // Limita o tamanho do arquivo (5MB)
      if (file.size > 5 * 1024 * 1024) {
        this.error = 'A imagem deve ter menos de 5MB.';
        return;
      }
      ...
      this.profileService.uploadProfileImage(file).subscribe({
        next: (response) => {
          // Formata a URL corretamente com o cache-busting
          this.profileImage = this.formatImageUrl(response.profileImage);
          this.isUploading = false;
          ...
          // Dispara um evento de atualização para o app saber que a imagem foi alterada
          const event = new CustomEvent('profile:imageUpdated', { 
            detail: { profileImage: this.profileImage }
          });
          window.dispatchEvent(event);
        },
        error: (error) => {
          console.error('Erro ao fazer upload da imagem:', error);
          this.error = 'Falha ao enviar imagem. Tente novamente.';
          this.isUploading = false;
  ...
  removeProfileImage(): void {
    ...
    this.profileService.removeProfileImage().subscribe({
      next: () => {
        ...
        // Dispara um evento de atualização para o app saber que a imagem foi removida
        const event = new CustomEvent('profile:imageUpdated', { 
