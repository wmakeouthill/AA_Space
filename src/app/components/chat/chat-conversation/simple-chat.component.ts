...
export class SimpleChatComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @ViewChild('messageInput') messageInputElementRef!: ElementRef<HTMLInputElement>; // Adicionado ViewChild para o input
  ...
  sendMessage(): void {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (this.messageInputElementRef?.nativeElement) {
